...
// This file contains altered versions of classes in the rocket chip hierarchy to allow for an SCIE implementation to
// be specified in a config.
// Any updates to the rocket chip codebase are likely to break this implementation.

// a trait to represent the SCIE input
trait scie_io extends HasBlackBoxInline{
    val io:SCIEPipelinedInterface

}

class SCIEModuleImp(outer: SCIETile,scieimp:(Int)=>scie_io) extends BaseTileModuleImp(outer)
    with HasMyFpuOpt
    with HasMyRoCCModule
    with HasICacheFrontendModule {
  Annotated.params(this, outer.rocketParams)

  val core = Module(new SCIERocket(outer,scieimp)(outer.p))
...
// class SCIETile(rocketParams: RocketTileParams,crossing: TileCrossingParamsLike,lookup: LookupByHartIdImpl)(implicit q:Parameters) 
// extends RocketTile(rocketParams,crossing,lookup)(q){
//   override lazy val module=new SCIEModuleImp(this,new BitRocket(this)(this.p))
// }
case class SCIETileParams(
    ...
    imp: (SCIETile)=>SCIEModuleImp,
    ) extends InstantiableTileParams[SCIETile] {
  require(icache.isDefined)
  require(dcache.isDefined)
  def instantiate(crossing: TileCrossingParamsLike, lookup: LookupByHartIdImpl)(implicit p: Parameters): SCIETile = {
    new SCIETile(this, crossing, lookup,(imp))
  }
}
...
class SCIETile private(
      val rocketParams: SCIETileParams,
    ...
    with HasICacheFrontend
{
  ...
  def this(params: SCIETileParams, crossing: TileCrossingParamsLike, lookup: LookupByHartIdImpl,imp:(SCIETile)=>SCIEModuleImp)(implicit p: Parameters) =
...
trait HasMyRoCCModule extends CanHavePTWModule
    with HasCoreParameters { this: SCIEModuleImp with HasMyFpuOpt =>
...
trait HasMyFpuOpt { this: SCIEModuleImp =>
...
object MyOMISA {
  def rocketISA(tile: SCIETile, xLen: Int, pgLevels: Int): OMISA = {
