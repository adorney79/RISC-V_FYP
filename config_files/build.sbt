...
lazy val chipyard = (project in file("generators/chipyard"))
  ...
  .dependsOn(rocketchip, boom, hwacha, sifive_blocks, sifive_cache, iocell,
    ...
    gemmini, icenet, tracegen, cva6, nvdla, sodor,risc_v_fyp)
  .settings(libraryDependencies ++= rocketLibDeps.value)
  .settings(commonSettings)
...
lazy val fpga_platforms = (project in file("./fpga"))
  ...
  .settings(commonSettings)
lazy val risc_v_fyp=(project in file("generators/RISC-V_FYP")).settings(commonSettings).dependsOn(rocketchip,chisel_testers)
  .sourceDependency(testchipip, testchipipLib)
  .settings(libraryDependencies ++= rocketLibDeps.value)
  .settings(commonSettings)